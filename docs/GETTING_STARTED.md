...
Stream32 supports four off-the-shelf displays. Confirm you have the right one
before flashing, because look-alike products are not interchangeable.

| Display | Confirm before you flash | Power and data |
| --- | --- | --- |
| Waveshare `ESP32-S3-Touch-LCD-4` | Silkscreen reads **Rev 3.0** or **Rev 4.0**; select the matching 4-inch profile | Single USB-C carries data and power |
| Waveshare `ESP32-S3-Touch-LCD-4.3B` | Exact **4.3B**, 800x480 model; hardware validated, but a local build is required until a release image is published | Single USB-C carries data and power |
...
| `ESP32-2432S028R` "Cheap Yellow Display" | It is the **R** (resistive touch) model. The panel controller varies between batches, so check the screen after the first flash | USB-C carries data and power |
